...
- Commit messages: never write the literal `[skip ci]` (GitHub skips ALL push workflows on that string — ci, gates, deploy). Refer to it as `skip-ci`; only the discovery-harvest bot commit may carry the real token.
