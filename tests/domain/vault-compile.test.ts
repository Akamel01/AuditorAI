...
describe("vault compile determinism (V2)", () => {
  ...
  it("import is byte-idempotent and compiles the seeded journal-free state", () => {
  ...
  it("unknown zones are ignored safely by import", () => {
    const tmp = mkdtemp();
    mkdirSync(path.join(tmp, "vault/journal"), { recursive: true }); // not chartered for import
    writeFileSync(
      path.join(tmp, "vault/journal/x.md"),
      "---\ntitle: t\ntype: journal\ndate: bad-date\nowner: agent\n---\n",
    );
