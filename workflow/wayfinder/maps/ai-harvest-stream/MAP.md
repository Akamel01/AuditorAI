...
- [Tickets H1-H6 created](tickets/H1-ai-provider-gpt5-nano.md): H1 provider; H2-structured-workflow.md; H3-control-api.md; H4-ui-monitoring.md; H5-verification-loop.md; H6-fixtures-samples.md.
...
- [Ticket index leniency (H13 closed 2026-09-10, validator GO)](tickets/H13-ticket-index-leniency.md): `loadTicketsFromTree` lenient (per-file try/catch → `console.warn` + `skipped:[{file,reason}]`, `TicketIndex.skipped` required, parsers stay strict) + `scripts/wayfinder-tickets.ts --lint/--root` (exit 0/1/2, `--json {counts}` preserved, M-R19 write removed all modes — ephemeral, zero readers) + pre-commit guarded step + `ci.yml quality` step + temp-dir 1+1 test; chain in `.autoforge/{architecture/H13,plans,execution/M-H13-*,reviews/M-H13-*,validation/H13-*}`.
- [H1-H6 closed 2026-09-10 (done-in-code, cross-evidence Resolutions)](tickets/H1-ai-provider-gpt5-nano.md): H1 live-legacy `ai-search` + canonical `agent-reach-search`; H2 `HarvestStream` state machine; H3 control API (+H9 delta); H4 mission-control UI; H5 `verifyStream`/tickStream; H6 `ai-harvest.test.ts` fixtures — each ticket carries `## Resolution` with current file:line anchors (see ticket files).
