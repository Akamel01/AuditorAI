...
- [Live-verify Run this gap (Live)](workflow/wayfinder/maps/harvest-verification/tickets/HV6-verify-gap.md): `job_mts45qbe_o5jb9a` `usa:DETAILED_DESIGN` gap-targeted `live:true` 22 logs `D01..D10` `degraded` `ledgerDelta 10` `healthAdvanced true` `dedupe degradedOk`, busy `job_mts44g7x_p7gu2l` `error harvest lock held` `skipped` — evidence `state/harvest-verify/HV6-*.json` + `HV6-latest.json`.
- [Live-verify Run Live Harvest](workflow/wayfinder/maps/harvest-verification/tickets/HV7-verify-harvest.md): `job_mttnvf2t_9dm7r2` gap-aware `cellKey null` `gaps_ranked.slice(0,3)` 22 logs `degraded` `ledger +10` `coverage.generated==ranAtIso` `queue[0..2]` matches `healthAdvanced true` — evidence `state/harvest-verify/HV7-*.json`.
...
- [Wire validation gates](workflow/wayfinder/maps/harvest-verification/tickets/HV8-validation.md): `package.json:ci:local` + `ci.yml quality` + `pre-commit` now run `node scripts/harvest-verify.mjs --mock`; `AGENTS.md:Guardrails` harvest-verify line added; Playwright `@harvest` post-deploy via `ADMIN_KEY`. 
